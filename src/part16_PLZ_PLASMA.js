...
// Release 1.05 update palette management and fade-in/out to solve https://github.com/covalichou/second-reality-js/issues/4 and reproduce slight palette management bug in original code
// In v1.05 palette are exactly as original
...
function PartInit()
{
    ...
    lc=60;
    sequence=0; 
    fadepal=new Array(768);
    fadepal.fill(0);
    initpparas(0);
    do_pal=126;  //first fade duration is different (from white to normal palette), exact duration to get original palette
...
{   
    ...
    else if (do_pal>=0) {increase_apply_palette(); do_pal--;}  //else increment palette until fade in is finished
...
function initpparas(sequence)  //init plasma parameters for a given sequence
{
        ...
        curpal=sequence;
        do_pal= 30;  //start palette fade(exact duration to get original palette)
        for (let i=0;i<768;i++)
           fadepal[i]= fadepal[i] & 0xFF;  // implement "the bug" in which fractional part is not cleared between 2 palette changes (clear only high 8 bits)
        for (let a=0;a<256;a++)    console.log ("clear ",fadepal[a*3],fadepal[a*3+1],fadepal[a*3+2]);
}
//************************************************************************************************************************************************************************************************************
function increase_apply_palette()  //manage palette in same way as original code
{
    let r,g,b;
    for (let a=0;a<768;a++) { fadepal[a]+=pals[curpal][a] }  //increment to fade in or out (8.8 fixed point)
    for (let a=0;a<256;a++)  
    {
        r= (fadepal[a*3  ]>>>8) & 0x3F;  // from 8.8 fixed point to 0..63 Vga level
        g= (fadepal[a*3+1]>>>8) & 0x3F;
        b= (fadepal[a*3+2]>>>8) & 0x3F;
...
//	RB-white Palette 3 (not used in final demo release version but present in original source code)
	pptr=3; pal=pals[3];
	for(a=1;a<64;a++) { pal[pptr++]=iptau(a  ) ;pal[pptr++]=iptau(0   );pal[pptr++]=iptau(0   ); }
...
//	white Palette 2
	pptr=3; pal=pals[2];
	for(a=1;a<64;a++) { pal[pptr++]=Math.floor(iptau(0   )/2);pal[pptr++]=Math.floor(iptau(0   )/2);pal[pptr++]=Math.floor(iptau(0   )/2); }
	for(a=0;a<64;a++) { pal[pptr++]=Math.floor(iptau(a   )/2);pal[pptr++]=Math.floor(iptau(a   )/2);pal[pptr++]=Math.floor(iptau(a   )/2); }
	for(a=0;a<64;a++) { pal[pptr++]=Math.floor(iptau(63-a)/2);pal[pptr++]=Math.floor(iptau(63-a)/2);pal[pptr++]=Math.floor(iptau(63-a)/2); }
	for(a=0;a<64;a++) { pal[pptr++]=Math.floor(iptau(0   )/2);pal[pptr++]=Math.floor(iptau(0   )/2);pal[pptr++]=Math.floor(iptau(0   )/2); }

//	white II Palette 4 (not used in final demo release version but present in original source code)
	pptr=3; pal=pals[4];
	...
	for(a=0;a<75;a++) { pal[pptr++]=Math.floor(iptau(a*64/75)*8/10); pal[pptr++]=Math.floor(iptau(a*64/75)*9/10); pal[pptr++]=iptau(a*64/75); }
    
	//convert for fade in/out
    for(a=0;a<768;a++) pals[0][a]=(pals[0][a]-63)*2;  //negative increment for first fade in (will fade-in from white in 127 frames)
	for (p=1;p<5;p++)
        for(a=0;a<768;a++) pals[p][a]=pals[p][a]*8;  //compute increment to apply to each frame (to be interpreted as 8.8 fixed point)!  multiply by 8 and divide later by 256, (=divide by 32 and get 8 bit fractional part) (will fade in from black in ~32 frames)
